...
const Video = () => {
  ...
  // Tipado  videoRef como HTMLVideoElement o null
  ...
  const handlePlayClick = () => {
    if (videoRef.current) {
      ...
      setIsPlaying(true); // Cambia el estado para que el ícono desaparezca
    }
  };
  ...
  return (
    <section className="max-container padding-container py-16">
      <div className="bg-video bg-cover bg-center bg-no-repeat h-[260px] md:h-[400px] xl:h-[500px] grid
        ...
        {!isPlaying && (
          <div 
            className="absolute h-20 w-20 bg-green-500 text-white flex items-center justify-center rounded-full cursor-pointer z-10"
            onClick={handlePlayClick} // Llama a handlePlayClick al hacer clic
          >
            <RiPlayFill size={36} /> {/* Aumenta el tamaño del ícono */}
          </div>
        )}
